...
class SonarrAdapterFailure(RuntimeError):
    ...
    def __repr__(self) -> str:
        return f"SonarrAdapterFailure(code={self.code!r}, retryable={self.retryable!r})"


@dataclass(frozen=True)
class SonarrCapabilities:
    api_version: int
...
@dataclass(frozen=True)
class SonarrSnapshot:
    capabilities: SonarrCapabilities
    system_status: SonarrSystemStatus
    root_folders: tuple[SonarrRootFolder, ...]
    download_clients: tuple[SonarrDownloadClient, ...]
    quality_profiles: tuple[SonarrQualityProfile, ...]
    queue: SonarrQueueSummary
...
class SonarrAdapter:
    ...
    def _read(self, path: str, *, version_probe: bool = False) -> dict[str, Any] | list[Any]:
        ...
        try:
            ...
            failure = SonarrAdapterFailure(code, retryable=error.retryable)
    ...
    def _status(self) -> tuple[SonarrCapabilities, SonarrSystemStatus]:
        payload = _mapping(self._read(f"{API_ROOT}/system/status", version_probe=True))
        if _string(payload, "appName").lower() != "sonarr":
            raise SonarrAdapterFailure("SERVICE_IDENTITY_INVALID")
    ...
    def discover_capabilities(self) -> SonarrCapabilities:
        capabilities, _ = self._status()
        return capabilities

    def _root_folders(self) -> tuple[SonarrRootFolder, ...]:
        output = []
    ...
    def _quality_profiles(self) -> tuple[SonarrQualityProfile, ...]:
        output = []
    ...
    def _queue_summary(self) -> SonarrQueueSummary:
        ...
        for page in range(1, MAX_QUEUE_PAGES + 1):
            path = (
                ...
                "&sortKey=timeleft&sortDirection=ascending&includeUnknownSeriesItems=true"
            )
            ...
            if (
                ...
                raise SonarrAdapterFailure("QUEUE_BOUNDS_INVALID")
            if expected_total is None:
                expected_total = total
            elif total != expected_total:
                raise SonarrAdapterFailure("QUEUE_PAGINATION_INVALID")
            for raw in records:
                item = _mapping(raw)
            ...
            if collected == total:
                return SonarrQueueSummary(total, tuple(sorted(status_counts.items())))
            if collected > total or not records:
                raise SonarrAdapterFailure("QUEUE_PAGINATION_INVALID")
        raise SonarrAdapterFailure("QUEUE_BOUNDS_INVALID")

    def read_snapshot(self) -> SonarrSnapshot:
        capabilities, system_status = self._status()
        return SonarrSnapshot(
